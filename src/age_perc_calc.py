...
def calc_filtered_counts_of_building_age(df, age_types):
    """
    ...
    unknown_count = df[(df['premise_age_bucketed'].isna()) | (df['premise_age_bucketed'] == 'Unknown date')    ]
    ...
    fa.update({'None_age': len(unknown_count)})
...
def process_postcode_building_age(pc, onsud_data, INPUT_GPK, overlap=False, batch_dir=None, path_to_pcshp=None):
    ...
    if uprn_match.empty or uprn_match is None:
        ...
        dc = calc_filtered_counts_of_building_age(df, age_types)
        if df is not None:
