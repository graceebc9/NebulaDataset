        ...
        for region in all_regions:
            ...
            for attr, df in dfs.items():
                ...
                for other_attr in other_attrs:
                    ...
                    if diff:
                        print(f"Inconsistency found in {attr} for region {region}: {diff}")
                        results['valid'] = False
        ...
        for region in all_regions:
            ...
            for batch in common_batches:
                ...
                if len(set(counts.values())) > 1:
                    results['valid'] = False
                    print(f"Count inconsistency for region {region}, batch {batch}: {counts}")
...
def print_validation_summary(variation_results: Dict, 
    ...
    if not variation_results.get('valid', False):
        logger.info("Regions missing variations:", 
    ...
    if not batch_results.get('valid', False):
        for attr, result in batch_results.items():
            if attr != 'valid' and not result.get('valid', True):
                logger.info(f"- {attr}: Invalid regions:", ', '.join(result.get('invalid_regions', [])))
                logger.info('erorrs: ', result.get('anomaly_details', []))
    ...
    if not consistency_results.get('valid', False):
        if consistency_results.get('region_consistency'):
            for attr, result in consistency_results['region_consistency'].items():
                if not result['valid']:
                    logger.info(f"- {attr} missing regions:", ', '.join(result['missing_regions']))
        if consistency_results.get('count_consistency', {}).get('inconsistencies'):
            logger.info("- Count inconsistencies found in:", 
    ...
    if all_valid:
        ...
        logger.info("✗ One or more validation checks failed. Please review the details above.")
...
def call_validations(default_length: int = 10000) -> None:
    ...
    """
    logger.info(f"Running validations with default batch size: {default_length}")
