...
def process_residential_counts(df: pd.DataFrame) -> pd.DataFrame:
    ...
    building_cols = ['clean_res_total_buildings', 'outb_res_total_buildings']
...
def calculate_energy_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['gas_eui_GIA_MR'] = df['total_gas'].div(df['clean_res_scaled_fl_area_meta_res_total'])
    # df['gas_eui_scaled_gross_epc'] = df['total_gas'].div(df['clean_res_scaled_fl_area_gross_epc_total'])

    df['elec_eui_GIA_MR'] = df['total_elec'].div(df['clean_res_scaled_fl_area_meta_res_total'])
    # df['elec_eui_scaled_gross_epc'] = df['total_elec'].div(df['clean_res_scaled_fl_area_gross_epc_total'])

    df['gas_per_meter'] = df['total_gas'] / df['num_meters_gas']
...
def process_floor_areas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    floor_area_cols = {
        'H': ['clean_res_total_fl_area_H_total', 'outb_res_total_fl_area_H_total' ],
        'FC': ['clean_res_total_fl_area_FC_total', 'outb_res_total_fl_area_FC_total']
    }
...
def post_proc_new_fuel(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Calculate floor area confidence
