...
def load_rural_uran():
    """ Load the rural urban classification data
    """
    rur_uran = pd.read_csv('src/global_avs/Rural_Urban_Classification_(2021)_of_Output_Areas_in_EW.csv')

    return rur_uran

def load_centroids():
    """'centroids_wgs84.csv'"""
    logger.info('Loading postcode centroids')
    centroids = pd.read_csv('src/global_avs/centroids_wgs84.csv')
    # centroids.drop(columns=['Unnamed: 0', 'PCD','PCD2', 'PCDS',] , inplace=True)
    return centroids