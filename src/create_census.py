...
def main(input_data_sources):
    ...
    census_loc = os.path.join(input_data_sources, '2021_UK_census')
...
def unify_census(input_data_sources_location):
    ...
    res.rename(columns={'Output Areas Code': 'OA21CD'}, inplace=True)
    res.to_csv('intermediate_data/unified_census_data.csv', index=False)
